<script lang="ts">
    import { CheckOutline, ChevronDoubleRightOutline, CloseOutline, MinusOutline, PlaySolid } from "flowbite-svelte-icons";
...
<div 
    class="relative h-6 w-6 flex items-center justify-center cursor-pointer" 
    onmouseenter={() => (showPlay = true)} 
    onmouseleave={() => (showPlay = false)}
    onclick={(e) => { e.stopPropagation(); player.play(); }}
    role="button"
    tabindex="0"
>
    {#if showPlay && name}
    ...
    {:else}
        <div in:fade={{ duration: 150 }} out:fade={{duration: 150}} class="absolute inset-0 flex items-center justify-center">
            {#if !status}
